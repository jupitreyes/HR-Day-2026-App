  ...
  if (unlocked) {
    return (
      <div className="flex flex-col flex-1 items-center justify-center space-y-12 animate-in fade-in zoom-in">
        ...
        <Button onClick={onNext}>Continue to Station 3</Button>
      </div>
    )
  }
    ...
    <div className="space-y-12 py-6">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          >
            {hintsUsed['st2-c1'] ? 'Hint Used' : 'Use Hint (+1 min)'}
          </Button>
        </div>
        
        {hintsUsed['st2-c1'] && (
          <div className="p-4 border border-retro-yellow text-retro-yellow bg-retro-yellow/10 font-sans">
            <strong className="font-bold">HINT:</strong> Start by notifying those impacted. A swarm helps regroup to fix the issue. "Go offline" is a distractor.
          </div>
        )}

        <div className="space-y-4">
          <p className="text-xl">Determine the correct escalation path order.</p>
          <div className="grid grid-cols-1 gap-4 font-sans">
            {[0, 1, 2].map((step) => (
              <div key={step} className="flex flex-col gap-2">
                <span className="font-bold text-sm text-retro-cyan">STEP {step + 1}</span>
                <select 
                  className="bg-retro-bg border-2 border-retro-cyan/50 text-white p-3 outline-none focus:border-retro-cyan w-full"
          ...
          {clue1Error && <p className="text-retro-pink animate-pulse font-sans font-bold">Incorrect escalation path in Clue 1.</p>}
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          >
            {hintsUsed['st2-c2'] ? 'Hint Used' : 'Use Hint (+1 min)'}
          </Button>
        </div>

        {hintsUsed['st2-c2'] && (
          <div className="p-4 border border-retro-yellow text-retro-yellow bg-retro-yellow/10 font-sans">
            <strong className="font-bold">HINT:</strong> Assess impact and inform stakeholders first. UAT happens before publishing resources, and comms go out right before go-live.
          </div>
        )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-2 font-sans">
            {clue2Order.map((item, index) => (
              <div key={item} className="flex items-center justify-between bg-white/5 p-3 border border-white/10 gap-3">
                <span className="font-semibold text-sm leading-tight flex-1">{item}</span>
                <div className="flex flex-col gap-1">
                  <button 
                    onClick={() => moveUp(index)} 
                    disabled={index === 0}
                    className="p-1 bg-retro-bg border border-retro-cyan/50 text-retro-cyan disabled:opacity-30"
                  >
                    <ArrowUp size={16} />
                  </button>
                  <button 
          ...
          {clue2Error && <p className="text-retro-pink animate-pulse font-sans font-bold">Incorrect checklist order in Clue 2.</p>}
        </div>
      </div>
